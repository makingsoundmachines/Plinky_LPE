...
// 0.9d - bunch of small fixes! encoder click toggles arp & latch; pressure display on right of screen; note display;
// arp & latch show correct values, not 0.0; encoder clicks toggles between default and 0; long encoder click clears all
// modulation; negative strides no longer allowed; add accelerometer sensitivity parameter; LATCH IS NOW PER PRESET! not
// global; audio in now causes cute ripples in the LEDs from the bottom left corner; fixes for LEDs including mod A src
// not lighting; when choosing a parameter, all mod src's that are active light up; when choosing a mod src, all params
// affected by that mod src light up; 0.9e - midi note allocation change 0.9f - midi refinements 0.9g - ?? 0.9h - new
// wavetable header 0.9i - possible fix for midi24ppqncounter 0.9j - first version of midi out, also expander outputs
// lfo vals 0.9k - refine midi out a bit 0.9l - add accel test 0.9m - stereo width, patch names & categories 0.9n -
